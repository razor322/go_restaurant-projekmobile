...
import 'package:go_restoran/screen/recipe/list_recipe_page.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    super.initState();
    getProduct().then((value) => getRecipe());
    getRecipe();
  }
  ...
  Future<void> getRecipe() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          _recipeList = modelRecipe.recipes ?? [];
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed Load data')),
                  ...
                  Padding(
                    ...
                    child: Container(
                      ...
                      child: ListView.builder(
                          ...
                          itemCount: 5,
                          itemBuilder: (context, index) {
                            Recipe dataRec = _recipeList[index];
                            return GestureDetector(
                              onTap: () {
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                        builder: (context) =>
                                            DetailRecipePage(dataRec, false)));
                              },
                              child: Padding(
                                padding: const EdgeInsets.only(right: 10),
                                child: Container(
